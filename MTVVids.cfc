...
Edited: Tuesday 11th November, 2008
Revision History:
added three methods (genreAlias, genreArtist, genreVideos) to coincide with update of the sixth release of the API
...
<cfcomponent displayname="MTVVids" hint="contains functions to access the MTVN video API">
			...
			<cfhttp url="#feedURL#" />
				...
				<cfif arraylen(ItemNodes) GT 0>
					...
					<cfif arguments.returnQuery EQ 'yes'>
						<cfset resultStr = QueryNew("artist_uri,name,videoID,description,keywords,mediaplayer,category,videoDuration,videoExpression,videoIsDefault,videoMedium,videoType,videoURL,credits,thumbnails,videoGenreLink,videoGenre")>
                        ...
                        <cfloop from="1" to="#arrayLen(itemsArr[rq].links)#" index="getGenre">
                            <cfif itemsArr[rq].links[getGenre].rel EQ 'http://api.mtvnservices.com/1/schemas/genre'>
                                <cfset temp = QuerySetCell(resultStr, "videoGenreLink", "#itemsArr[rq].links[getGenre].href#", #rq#)>
                        		<cfset temp = QuerySetCell(resultStr, "videoGenre", "#itemsArr[rq].links[getGenre].title#", #rq#)>
                            </cfif>
                        </cfloop>
		...
		<cfargument name="returnQuery" required="no" type="string" default="no" hint="if yes, the results will be returned as a query recordset. Default is 'no'" />
			...
			<cfhttp url="#feedURL#" />
				...
				<cfif arraylen(ItemNodes) GT 0>
					...
					<cfif arguments.returnQuery EQ 'yes'>
						<cfset resultStr = QueryNew("name, videoID, description,keywords,videoDuration,videoExpression,videoIsDefault,videoMedium,videoType,videoURL,category,credits,mediaPlayer,thumbnails,videoGenreLink,videoGenre")>
						<cfset newRow = QueryAddRow(resultStr, #arrayLen(itemsArr)#)>
						<cfloop from="1" to="#arrayLen(itemsArr)#" index="rq">
							<!--- Set the values of the cells in the query --->
			...
			<cfhttp url="#feedURL#" />
				...
				<cfif arraylen(ItemNodes) GT 0>
					...
					<cfif arguments.returnQuery NEQ 'no'>
						<cfset resultStr = QueryNew("videoID, title, thumbnails, credits, category, artist_uri, artistrelatedLink, artistvideoLink, videoDuration, videoExpression, videoIsDefault, videoMedium, videoType, videoURL,videoGenreLink, videoGenre")>
                    ...
                    <cfloop from="1" to="#arrayLen(videoItemsArr[rq].links)#" index="getGenre">
                        <cfif videoItemsArr[rq].links[getGenre].rel EQ 'http://api.mtvnservices.com/1/schemas/genre'>
                            <cfset temp = QuerySetCell(resultStr, "videoGenreLink", "#videoItemsArr[rq].links[getGenre].href#", #rq#)>
                            <cfset temp = QuerySetCell(resultStr, "videoGenre", "#videoItemsArr[rq].links[getGenre].title#", #rq#)>
                        </cfif>
                    </cfloop>
		...
		<cfargument name="returnQuery" required="no" type="string" default="no" hint="if yes, the results will be returned as a query recordset. Default is 'no'." />
			...
			<cfif arrayLen(rootNodes) GT 0>
				...
				<cfif arraylen(ItemNodes) GT 0>
					...
					<cfif arguments.returnQuery EQ 'yes'>
						<cfset resultStr = QueryNew("artist_uri,name,videoID,category,videoDuration,videoExpression,videoIsDefault,videoMedium,videoType,videoURL,credits,thumbnails,videoGenre,videoGenreLink")>
						<cfloop from="1" to="#arrayLen(itemsArr)#" index="rq">
                            ...
                            <cfloop from="1" to="#arrayLen(itemsArr[rq].links)#" index="getGenre">
                                <cfif itemsArr[rq].links[getGenre].rel EQ 'http://api.mtvnservices.com/1/schemas/genre'>
                                    <cfset temp = QuerySetCell(resultStr, "videoGenre", "#itemsArr[rq].links[getGenre].title#")>
                                    <cfset temp = QuerySetCell(resultStr, "videoGenreLink", "#itemsArr[rq].links[getGenre].href#")>
                                </cfif>
                            </cfloop>
    ...
    <cffunction name="genreAlias" access="public" output="true" returnType="any">			
		<cfargument name="genre_alias" required="yes" type="string" hint="the specific genre alias (eg. http://api.mtvnservices.com/1/genre/[genre_alias]/) " />
		<cfargument name="returnQuery" required="no" type="string" default="no" hint="if yes, the results will be returned as a query recordset. Default is 'no'." />
			<cfset var searchResults = ''>
			<cfset var returnedXML = ''>
			<cfset var resultStr = ''>
			...
			<cfhttp url="#arguments.genre_alias#" />
			...
			<cfif arrayLen(rootNodes) GT 0>
				...
				<cfif arraylen(ItemNodes) GT 0>
					...
					<cfif arguments.returnQuery EQ 'yes'>
						<cfset resultStr = QueryNew("Name,genreAlias, genreArtist, genreVideos")>
						<cfloop from="1" to="#arrayLen(itemsArr)#" index="rq">
							<cfif structKeyExists(itemsArr[rq], "links")>
							<cfloop from="1" to="#arrayLen(itemsArr)#" index="qv">
							<cfset newRow = QueryAddRow(resultStr)>
	                            
							<cfset temp = QuerySetCell(resultStr, "Name", "#itemsArr[rq].title#")>
							<cfset temp = QuerySetCell(resultStr, "genreAlias", "#itemsArr[rq].links[1].href#")>
                            <cfset temp = QuerySetCell(resultStr, "genreArtist", "#itemsArr[rq].links[2].href#")>
                            <cfset temp = QuerySetCell(resultStr, "genreVideos", "#itemsArr[rq].links[3].href#")>
    ...
    <cffunction name="genreArtist" access="public" output="true" returnType="any" description="Returns a list of artists for a specified genre.">			
		<cfargument name="genre_alias" required="yes" type="string" hint="the specific genre alias (eg. http://api.mtvnservices.com/1/genre/[genre_alias]/artists/) " />
		<cfargument name="returnQuery" required="no" type="string" default="no" hint="if yes, the results will be returned as a query recordset. Default is 'no'." />
			<cfset var searchResults = ''>
			<cfset var returnedXML = ''>
			<cfset var resultStr = ''>
			...
			<cfif arrayLen(rootNodes) GT 0>
				...
				<cfif arraylen(ItemNodes) GT 0>
					...
					<cfif arguments.returnQuery EQ 'yes'>
						...
						<cfset resultStr = QueryNew("artist_uri,artistName,artistVideoLink,artistRelatedLink,genreAlias,videos")>

                        <cfloop from="1" to="#arrayLen(itemsArr)#" index="rq">
                        	<cfset newRow = QueryAddRow(resultStr)>
							<cfset temp = QuerySetCell(resultStr, "artist_uri", "#itemsArr[rq].ID#")>
                            <cfset temp = QuerySetCell(resultStr, "artistName", "#itemsArr[rq].title#")>
                            
                            <cfloop from="1" to="#arrayLen(itemsArr[rq].links)#" index="getLinks">
                                <cfif itemsArr[rq].links[getLinks].rel EQ 'http://api.mtvnservices.com/1/schemas/artist/videos'>
                                    <cfset temp = QuerySetCell(resultStr, "artistVideoLink", "#itemsArr[rq].links[getLinks].href#")>
                                <cfelseif itemsArr[rq].links[getLinks].rel EQ 'http://api.mtvnservices.com/1/schemas/artist/related'>
									<cfset temp = QuerySetCell(resultStr, "artistRelatedLink", "#itemsArr[rq].links[getLinks].href#")>
								<cfelseif itemsArr[rq].links[getLinks].rel EQ 'http://api.mtvnservices.com/1/schemas/genre'>
                                	<cfset temp = QuerySetCell(resultStr, "genreAlias", "#itemsArr[rq].links[getLinks].href#")>
								</cfif>
                            </cfloop>
                            
                            <cfset temp = QuerySetCell(resultStr, "videos", "#itemsArr[rq].videos#")>
    ...
    <cffunction name="genreVideos" access="public" output="true" returnType="any" description="Returns a list of videos for a specified genre.">			
		<cfargument name="genre_alias" required="yes" type="string" hint="the specific genre alias (eg. http://api.mtvnservices.com/1/genre/hip_hop/videos/) " />
		<cfargument name="returnQuery" required="no" type="string" default="no" hint="if yes, the results will be returned as a query recordset. Default is 'no'." />
			<cfset var searchResults = ''>
			<cfset var returnedXML = ''>
			<cfset var resultStr = ''>
			...
			<cfif arrayLen(rootNodes) GT 0>
				...
				<cfif arraylen(ItemNodes) GT 0>
					<cfloop from="1" to="#arraylen(ItemNodes)#" index="i">
						...
						<cfset ItemStr.title = #ItemXML.entry.title.XmlText#>
						<cfset ItemStr.id = #ItemXML.entry.id.XmlText#>
                        <cfset ItemStr.artist = #ItemXML.entry.author.name.XmlText#>
                        <cfset ItemStr.description = #ItemXML.entry.content.XmlText#>
                        <cfset ItemStr.published = #ItemXML.entry.published.XmlText#>
						
                        <!--- get the media content data --->
                        <cfset contentNodes = xmlSearch(#ItemXML#,'/entry/content/')>
						<cfif arraylen(contentNodes) GT 0>
							<cfloop from="1" to="#ArrayLen(contentNodes)#" index="k">
								<cfset ContentXML = xmlparse(contentNodes[k])>
						...
						<cfloop from="1" to="#ArrayLen(linkNodes)#" index="k">
							...
							<!--- this is required to use other services, such as related artists --->					
							<cfif structKeyExists(#LinkXML.link.XmlAttributes#, "rel") AND #LinkXML.link.XmlAttributes.rel# EQ 'http://api.mtvnservices.com/1/schemas/artist'>
								<cfset ItemStr.artistLink = "#LinkXML.link.XmlAttributes.href#">
							</cfif>
					...
					<cfif arguments.returnQuery EQ 'yes'>
						...
						<cfset resultStr = QueryNew("artist_uri,artistName,title,videoID,category,videoDuration,videoExpression,videoIsDefault,videoMedium,videoType,videoURL,credits,thumbnails,videoGenre,videoGenreLink,published,description")>
						<cfloop from="1" to="#arrayLen(itemsArr)#" index="rq">
                        	<cfset newRow = QueryAddRow(resultStr)>
							
                            <cfset temp = QuerySetCell(resultStr, "artist_uri", "#itemsArr[rq].artistLink#")>
							<cfset temp = QuerySetCell(resultStr, "artistName", "#itemsArr[rq].artist#")>
                            <cfset temp = QuerySetCell(resultStr, "title", "#itemsArr[rq].title#")>
                            <cfset temp = QuerySetCell(resultStr, "published", "#itemsArr[rq].published#")>
                            <cfset temp = QuerySetCell(resultStr, "description", "#itemsArr[rq].description#")>
                            
                            <cfset temp = QuerySetCell(resultStr, "videoID", "#itemsArr[rq].id#")>
                            <cfset temp = QuerySetCell(resultStr, "videoDuration", "#itemsArr[rq].mediacontent.duration#")>
							<cfset temp = QuerySetCell(resultStr, "videoExpression", "#itemsArr[rq].mediacontent.expression#")>
							<cfset temp = QuerySetCell(resultStr, "videoIsDefault", "#itemsArr[rq].mediacontent.isdefault#")>
							<cfset temp = QuerySetCell(resultStr, "videoMedium", "#itemsArr[rq].mediacontent.medium#")>
							<cfset temp = QuerySetCell(resultStr, "videoType", "#itemsArr[rq].mediacontent.type#")>
							<cfset temp = QuerySetCell(resultStr, "videoURL", "#itemsArr[rq].mediacontent.url#")>
                            
                            <cfset temp = QuerySetCell(resultStr, "category", "#itemsArr[rq].mediacategory#")>
                            <cfset temp = QuerySetCell(resultStr, "credits", "#itemsArr[rq].mediacredits#")>
							<cfset temp = QuerySetCell(resultStr, "thumbnails", "#itemsArr[rq].mediathumbnails#")>
